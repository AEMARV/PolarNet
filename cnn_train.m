...
for t=1:opts.batchSize:numel(subset)
    ...
    for s=1:opts.numSubBatches
        ...
        if opts.usePolar  && opts.polarOpts.useUncertainty
            ...
            net.layers{1}.epoch = state.epoch;
        end
        ...
        res = vl_simplenn(net, im,DataParam, dzdy, res, ...
            ...
            'sync', opts.sync, ...
            'cudnn', opts.cudnn) ;
        if  strcmp(mode,'train') && net.layers{1}.useLocator
            net.layers{1}.LocatorNet = res(1).aux;
            if  state.epoch > 5
                state.imdb = setDataParamImdb(state.imdb,res(2).DataParam,batch,isFliped);
